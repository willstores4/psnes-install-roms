...
LIBS    := -lc -lkernel -lSceSysmodule -lSceSystemService -lSceCommonDialog -lSceMsgDialog
...
$(TARGET): $(ODIR) $(OBJS)
	...
	$(OO_PS4_TOOLCHAIN)/bin/linux/create-fself -in=$(ODIR)/$(TARGET).elf -out=$(ODIR)/$(TARGET).oelf --eboot $(TARGET) --paid 0x3800000000000011 --authinfo 
